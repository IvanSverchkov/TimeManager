import type { Task } from "@state/Task";
import { getDateKey } from "@utils/time";

type GetWidgetsDataParams = {
  tasks: Array<Task>;
};

export type WidgetsData = {
  taskCount: number;
  completedTasks: number;
  completedStoryPoints: number;
  requiredSeconds: number;
  tasksTrackedSeconds: number;
  tasksRequiredSeconds: number;
  timersTrackedSeconds: number;
  timersRequiredSeconds: number;
  totalStoryPoints: number;
  todaySeconds: number;
  totalSeconds: number;
};

export function getWidgetsData({ tasks }: GetWidgetsDataParams): WidgetsData {
  const todayKey = getDateKey();

  let totalSeconds = 0;
  let todaySeconds = 0;
  let completedTasks = 0;
  let completedStoryPoints = 0;
  let requiredSeconds = 0;
  let tasksTrackedSeconds = 0;
  let tasksRequiredSeconds = 0;
  let timersTrackedSeconds = 0;
  let timersRequiredSeconds = 0;
  let totalStoryPoints = 0;

  tasks.forEach((task) => {
    totalSeconds += task.seconds;
    todaySeconds += task.dailySeconds[todayKey] ?? 0;

    if (task.status === undefined) {
      timersTrackedSeconds += task.seconds;
    } else {
      tasksTrackedSeconds += task.seconds;
    }

    if (task.status !== "done") {
      const taskRequiredSeconds = Math.max(
        task.estimatedHours * 60 * 60 - task.seconds,
        0,
      );

      requiredSeconds += taskRequiredSeconds;
      if (task.status === undefined) {
        timersRequiredSeconds += taskRequiredSeconds;
      } else {
        tasksRequiredSeconds += taskRequiredSeconds;
      }
    }
    if (task.storyPoints !== undefined) {
      totalStoryPoints += task.storyPoints;
    }

    if (task.status === "done") {
      completedTasks += 1;
      completedStoryPoints += task.storyPoints ?? 0;
    }
  });

  return {
    completedTasks,
    completedStoryPoints,
    requiredSeconds,
    taskCount: tasks.filter((task) => task.status !== undefined).length,
    tasksTrackedSeconds,
    tasksRequiredSeconds,
    timersTrackedSeconds,
    timersRequiredSeconds,
    todaySeconds,
    totalStoryPoints,
    totalSeconds,
  };
}
